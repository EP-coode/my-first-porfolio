...
const canvas = /** @type {HTMLCanvasElement} */ (
  document.querySelector("#banner_canvas")
);
...
const hue_path =
  HUE_END > HUE_START ? HUE_END - HUE_START : 360 - HUE_START + HUE_END;
function nextColor() {
...
window.addEventListener("mousemove", (e) => {
  ...
  mouse_pos.nonRelative.x = e.x;
  mouse_pos.nonRelative.y = e.y;
  mouse_pos.x = Math.max(e.x - rect.left, 0);
  mouse_pos.y = Math.max(e.y - rect.top, 0);
});

window.addEventListener("scroll", (e) => {
  const rect = canvas.getBoundingClientRect();

  mouse_pos.x = Math.max(mouse_pos.nonRelative.x - rect.left, 0);
  mouse_pos.y = Math.max(mouse_pos.nonRelative.y - rect.top, 0);
});
...
class Particle {
  ...
  isVisable() {
    ...
    return (
      this.size > 0 && canvas.width > x && canvas.height > y && x > 0 && y > 0
    );
  }
  ...
  update(deltaTime) {
    ...
    const r_sqr = Math.pow(mouse_pos.x - x, 2) + Math.pow(mouse_pos.y - y, 2);
    this.force.x =
      (MOUSE_FORCE_FACTOR * (x - mouse_pos.x)) / Math.max(r_sqr, 10) +
      ENV_FORCE.x;
    this.force.y =
      (MOUSE_FORCE_FACTOR * (y - mouse_pos.y)) / Math.max(r_sqr, 10) +
      ENV_FORCE.y;
    ...
    if (this.size > this.starve_rate) this.size -= this.starve_rate;
    else this.size = 0;
  }
...
function isMouseInCanvas() {
  return (
    mouse_pos.x < canvas.width &&
    mouse_pos.x > 0 &&
    mouse_pos.y < canvas.height &&
    mouse_pos.y > 0
  );
}
...
function animate() {
  ...
  particles.forEach((p) => {
    if (!p.isVisable() && _isMouseinCanvas)
      p.respawn(
        mouse_pos.x + Math.random() - 0.5,
        mouse_pos.y + Math.random() - 0.5
      );
  });
